...
from bigpt.utils.parse_xml import XmlPage

class XmlBrowserEngine:
    def __init__(
        self,
        engine: WebBrowserEngineType | None = None,
        run_func: Callable[..., Coroutine[Any, Any, XmlPage | list[XmlPage]]] | None = None,
    ):
        engine = engine or CONFIG.web_browser_engine
        if engine is None:
            raise NotImplementedError
    ...
    @overload
    async def run(self, url: str) -> XmlPage:
        ...

    @overload
    async def run(self, url: str, *urls: str) -> list[XmlPage]:
        ...

    async def run(self, url: str, *urls: str) -> XmlPage | list[XmlPage]:
        return await self.run_func(url, *urls)